 ...
 * 
 * This system replaces naive img.onload detection with pixel-verified,
 * feature-level decode probing to prevent incorrect native/WASM routing decisions.
 */
...
export interface PixelComparisonResult {
  readonly x: number;
  readonly y: number;
  readonly expected: { r: number; g: number; b: number; a: number };
  readonly actual: { r: number; g: number; b: number; a: number };
